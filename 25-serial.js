            ...
            get:function(serialConfig) {
                ...
                connections[id] = (function() {
                    ...
                    var setupSerial = function() {
                    	var olderr = "";
                    	var realSerialPort = port
	                    if (realSerialPort.indexOf("GLOBAL") != -1) {
	                    	//realSerialPort = realSerialPort.replace("GLOBAL", "")
	                    	if (globalNode) {
		                    	var globalContext = globalNode.context().global;
		                    	//console.log("globalContext", globalContext.get)
		                    	var realPort = globalContext.get(realSerialPort)
		                    	//console.log("realPort", realSerialPort, realPort)
		                    	if (realPort != "" && realPort != undefined) {
		                    		realSerialPort = realPort
		                    	}  else {
		                    		realSerialPort = "UNKNOWN" + realSerialPort;
		                    	}
		                    }
	                    }
	                    console.log("port", realSerialPort)
	                    
                        obj.serial = new serialp(realSerialPort,{
                            baudRate: baud,
                            dataBits: databits,
                            parity: parity,
                            stopBits: stopbits,
